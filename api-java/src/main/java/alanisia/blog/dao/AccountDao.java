...
public interface AccountDao {
  @Insert("insert into account(role_id, email, username, password, create_at, update_at)" +
  ...
  @Update("update account set role_id = #{account.roleId}, email = #{account.email}, " +
      "username = #{account.username}, password = #{account.password}, update_at = #{account.updateAt}" +
      " where id = #{id}")
